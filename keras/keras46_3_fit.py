...
hist = model.fit(xy_train[0][0], xy_train[0][1], epochs=10, validation_split=0.2, batch_size=32) #만약 배치를 최대로 잡았다면 이 표현도 가능
# hist = model.fit(xy_train, epochs=10, steps_per_epoch=32,
...
plt.plot(hist.history['val_accuracy'], marker = '.' ,c = 'black', label = 'val_accuracy')
plt.grid() # 모눈종이에 하겠다
plt.title('keras47_3_fit')#제목
plt.ylabel('loss')#y축 이름
plt.xlabel('epochs')#x축 이름
plt.legend(loc='upper right') # upper right: 위쪽 상단에 표시하겠다.(라벨 이름들)
plt.show()# 보여줘

# loss :  0.0946432575583458
# val_loss : 0.9598766565322876
# accuracy :  0.96875
